...
export default function ForecastSettings({
  ...
  const [selectedUseCase, setSelectedUseCase] =
    useState<keyof typeof USE_CASES>("stable");
  ...
  useEffect(() => {
    if (validationResult?.valid && validationResult.info) {
      ...
      const newConfig = {
        ...config,
        ...recommendedConfig,
        seasonal_length: insights.seasonalityDetected
          ? recommendedConfig.seasonal_length
          : null,
      };
      setConfig(newConfig);
  ...
  useEffect(() => {
    if (!isAdvancedMode) {
      ...
      const newConfig: ForecastConfig = {
        ...
        seasonal_length: dataInsights?.seasonalityDetected
          ? useCaseConfig.seasonal_length
          : null,
      };
      setConfig(newConfig);
      onConfigChange(newConfig);
  ...
  const analyzeData = (
    info: UploadResponse["validation"]["info"]
  ): DataInsights => {
    ...
    const volatility: DataInsights["volatility"] =
      dataPoints < 30 ? "high" : "medium";
    ...
    if (!hasDateColumn) {
      warnings.push(
        "No date column detected - using row order for time sequence"
      );
    }
    ...
    if (dataPoints >= 90) {
      recommendations.push(
        "Excellent data history - all forecasting methods available"
      );
    }
    ...
    if (dataPoints >= 60) {
      recommendations.push(
        "Machine learning models may capture complex patterns in your data"
      );
    }
      ...
      <Card className="border-gray-200">
        ...
        <CardContent>
          <div className="flex items-center gap-2 text-gray-500">
            ...
            <span>
              Upload and validate your data to configure forecast settings
            </span>
          </div>
        </CardContent>
      </Card>
        ...
        <Card className="border-blue-100 bg-blue-50/30">
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="text-center">
                <div
                  className={`text-sm font-medium px-2 py-1 rounded border ${getDataQualityColor(
                    dataInsights.dataQuality
                  )}`}
                >
              ...
              <div className="text-center">
                <div className="text-sm font-medium text-purple-600">
                  {dataInsights.seasonalityDetected
                    ? "Detected"
                    : "Not Detected"}
                </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-gray-600 mb-3">
                  We&apos;ll automatically configure the best forecasting
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {Object.entries(USE_CASES).map(([key, useCase]) => (
                  <div
                    ...
                    onClick={() =>
                      setSelectedUseCase(key as keyof typeof USE_CASES)
                    }
                  >
              ...
              {dataInsights &&
                selectedUseCase === dataInsights.recommendedUseCase && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="AutoARIMA">
                        Auto ARIMA (Recommended)
                      </SelectItem>
                      <SelectItem value="AutoETS">
                        Auto ETS (Seasonal)
                      </SelectItem>
                      <SelectItem value="SeasonalNaive">
                        Seasonal Naive (Simple)
                      </SelectItem>
                      <SelectItem value="Naive">Naive (Basic)</SelectItem>
                      <SelectItem value="RandomWalkWithDrift">
                        Random Walk (Trending)
                      </SelectItem>
                      <SelectItem value="SklearnModel">
                  ...
                  <div className="space-y-2">
                    <Slider
                      ...
                      onValueChange={([value]: number[]) =>
                        handleConfigChange("horizon", value)
                      }
                      min={7}
                      max={365}
                      step={7}
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    <Slider
                      ...
                      onValueChange={([value]: number[]) =>
                        handleConfigChange("confidence_level", value / 100)
                      }
                      min={80}
                      max={99}
                      step={1}
                    ...
                    <div className="flex justify-between text-xs text-gray-500">
                      ...
                      <span className="font-medium">
                        {Math.round((config.confidence_level || 0.95) * 100)}%
                      </span>
                      <span>99%</span>
                    </div>
                  </div>
